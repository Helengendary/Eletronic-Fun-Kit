...
# Definir pinos SCL e SDA para ESP32
...
try:
    while True:
        ...
        # Obter valores IR e RED
        ir_value = max30100.ir
        red_value = max30100.red

        # Calcular batimento cardíaco (HR)
        if max30100.buffer_ir and len(max30100.buffer_ir) > 0:
            ir_peak = max(max30100.buffer_ir)
            ir_mean = sum(max30100.buffer_ir) / len(max30100.buffer_ir)
            hr = 60 * (ir_peak - ir_mean) / (ir_mean)
        else:
            hr = 0

        # Calcular saturação de oxigênio (SpO2)
        if max30100.buffer_ir and max30100.buffer_red and len(max30100.buffer_ir) > 0:
        ...
        print("Saturação de Oxigênio (SpO2): {:.2f}%".format(spo2))
